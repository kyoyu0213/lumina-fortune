...
export function buildDailyFortunePrompt(
  ...
  return [
    ...
    "",
    "## 最重要ルール：汎用ホロスコープ禁止",
    "- どのカードでも使えるような一般的な占い文章を書いてはいけない",
    "- 各セクションは、引いたカード固有の象徴・ストーリー・絵柄から導かれる内容でなければならない",
    "- 「心を整えましょう」「バランスを大切に」のような、カードを入れ替えても成立する文は禁止",
    "- 文章を書いた後に自問する：この文は別のカードでもそのまま使えるか？使えるなら書き直す",
    "",
    "### intro（冒頭導入 — 必須）",
    "- 鑑定結果の冒頭に必ず入れる導入文",
    "- ユーザー名、今日の曜日、今日のカード名を自然に含める",
    "- 昨日のカード情報がある場合は、昨日のカードから今日のカードへの流れ・つながりを自然に書く",
    "- 昨日の意味を長く説明しない。「昨日の〇〇から今日は〇〇へ」のように橋渡しとして短く触れる",
    "- 昨日のカード情報がない場合は昨日の言及を省略する",
    "- 昨日のカードがある場合、二日間の流れを「旅」のように読む意識を持つ",
    ...
    "",
    "### energy（今日のエネルギー — 必須）",
    "- 今日一日の感情的・状況的な空気感を1〜2文で書く",
    "- カードの絵柄やストーリーから連想される雰囲気を言葉にする",
    "- 例：戦車なら「前に進みたい気持ちが胸の中で膨らむ一日」",
    "- 例：月なら「少しぼんやりとした不安が漂うけれど、それは朝霧のようにやがて薄れていくもの」",
    "- overallFlow（一日の流れ）とは異なる。こちらは「空気」「気配」「感覚」",
    "",
    "### cardMeaning",
    ...
    "",
    "### closing（クロージング — 必須）",
    "- 鑑定の最後を締めるやさしい一言。1文、20〜40文字程度",
    "- 読んだ人が「今日も大丈夫」と思える安心感のある言葉",
    "- todayHint や whiteHint とは重複しない内容にする",
    "- 例：「今日のあなたの一歩一歩に、静かな光が寄り添っています。」",
    "- 例：「焦らなくて大丈夫。今日の流れは、ちゃんとあなたの味方です。」",
    "",
    "## 禁止",
    "- 各項目で似た内容を繰り返すこと",
